	...
	public static function getTempFile( $allow_memory = true ) {
		...
		if ( $allow_memory ) {
			...
			if ( ! $tmpdir || ! is_dir( $tmpdir ) || ! is_writable( $tmpdir ) ) {
				if ( defined( 'IDEAPARK_UPLOAD_DIR' ) ) {
					$tmpdir = IDEAPARK_UPLOAD_DIR;
				} elseif ( defined( 'TEMPLINES_THEME_UPLOAD_DIR' ) ) {
					$tmpdir = TEMPLINES_THEME_UPLOAD_DIR;
				}
			}
			$f = fopen( tempnam( $tmpdir, "fnt" ), "rb+" );
